...
from marinegym.actuators.fin200 import fin200
from marinegym.actuators.m600 import M600
...
@dataclass
class UnderwaterVehicleFinCfg(RobotCfg):
    force_sensor: bool = False
    
class UnderwaterVehicleFin(RobotBase):
    
    param_path: str
    DEFAULT_CONTROLLER: Type = LeePositionController
    cfg_cls = UnderwaterVehicleFinCfg

    def __init__(
        self, 
        name: str = None, 
        cfg: UnderwaterVehicleFinCfg=None, 
        ...
        self.num_fins = self.params["fin_configuration"]["num_fins"]

        self.action_spec = BoundedTensorSpec(-1, 1, self.num_rotors+int(self.num_fins/2), device=self.device)
        ...
        if self.cfg.force_sensor:
            ...
            state_dim = 19 + self.num_rotors + self.num_fins + 6
        else:
            self.use_force_sensor = False
            state_dim = 19 + self.num_rotors + self.num_fins
    ...
    def initialize(
        ...
        if self.is_articulation:
            ...
            fin_joint_indices = [
                i for i, dof_name in enumerate(self._view._dof_names)
                if dof_name.startswith("fin")
            ]
            if len(fin_joint_indices):
                self.fin_joint_indices = torch.tensor(
                    fin_joint_indices,
                    device=self.device
                )
            else:
                self.fin_joint_indices = None
        else:
        ...
        self.rotors = M600(rotor_config, dt=self.dt).to(self.device)
        ...
        self.fins_view = RigidPrimView(
            prim_paths_expr=f"{self.prim_paths_expr}/fin_*",
            name="fins",
            shape=(*self.shape, self.num_fins)
        )
        self.fins_view.initialize()
        fin_joint_indices = [
                i for i, dof_name in enumerate(self._view._dof_names) 
                if dof_name.startswith("fin")
            ]
        self.fin_joint_indices = torch.tensor(fin_joint_indices,device=self.device)
        self.arm_pos = torch.zeros(*self.shape,self.num_fins,device=self.device).squeeze(1)
        fin_config = self.params["fin_configuration"]
        self.fins = fin200(fin_config, dt=self.dt).to(self.device)
        fin_params = make_functional(self.fins)
        self.fin_params = fin_params.expand(self.shape).clone()

        self.tau_up = self.rotor_params["tau_up"]
        self.tau_down = self.rotor_params["tau_down"]
        self.throttle = torch.cat([self.rotor_params["throttle"], self.fin_params["throttle"]], dim=-1)
    ...
    def apply_action(self, actions: torch.Tensor) -> torch.Tensor:
        rotor_cmds = actions[...,0:self.num_rotors].expand(*self.shape, self.num_rotors)   
        ...
        fin_pos, fin_rot = self.fins_view.get_world_poses()
        fin_vel = self.fins_view.get_velocities()
        fin_cmds = torch.stack([actions[..., self.num_rotors],
                                actions[..., self.num_rotors + 1],
                                -actions[..., self.num_rotors],
                                -actions[..., self.num_rotors + 1]
                                ], dim=-1).expand(*self.shape, self.num_fins)

        fin_force, fin_angle = vmap(vmap(self.fins, randomness="different"), randomness="same")(fin_cmds, flow_vels, fin_pos, fin_rot, fin_vel, self.fin_params)
        fin_force = torch.clamp(fin_force, -30, 30)

        self.set_fin_poses(fin_angle.squeeze(1))
        ...
        self.fins_view.apply_forces_and_torques_at_pos(
            fin_force.reshape(-1, 3),
            is_global=False
        )
    ...
    def set_fin_poses(self, fin_pos):
        from omni.isaac.core.utils.types import ArticulationActions
        view_action = ArticulationActions(joint_positions=fin_pos,joint_indices=self.fin_joint_indices)
    ...
    def make(drone_model: str, controller: str=None, device: str="cpu"):
        drone_cls = UnderwaterVehicleFin.REGISTRY[drone_model]
        drone = drone_cls()
